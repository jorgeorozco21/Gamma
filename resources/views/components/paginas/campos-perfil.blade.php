...
<div class="max-w-6xl mx-auto bg-white rounded-[30px] p-8 md:p-10 shadow-sm border border-gray-50">
    ...
    <form method="POST" onsubmit="return confirm('¿Estás seguro de que deseas cambiar la contraseña?')" action="{{ url('/perfil/cambiar-contrasena') }}" class="mt-10 border-t pt-8 grid grid-cols-1 md:grid-cols-3 gap-x-12 gap-y-8">
        ...
        <div class="pt-8 border-t border-gray-100 flex flex-col md:flex-row justify-center items-center gap-4">
            <!-- Cambiar Contraseña -->
            <button type="submit" class="flex items-center gap-2 bg-purple-50 text-[#7B1FA3] hover:bg-purple-600 hover:text-white px-6 py-3 rounded-lg font-bold text-[10px] uppercase tracking-[0.15em] transition-all active:scale-95">
